...
I feel like most of the sentiment polarities were detected quite well and correct but there are a couple of wall inscriptions that seem to need further investigation. The result of the third version of the CIL IV 4160 (“_fasti_”) seems a bit odd. It has been marked as -1 negative. As can be seen from the final dataset, the original wall inscription “_fasti_” is a plural and in such a form means “calendar” in English. However, the lemma of it “_fastus_”, that can be found already in _LatinAffectus_ both as an adjective and a noun, can have several different meanings. Just some of the meanings are “_pride_”, “_arrogance_” or “_allowed_ / _not forbidden_”. It seems like the lemma “_fastus_” has been interpreted in _LatinAffectus_ only as “_pride_” or “_arrogance_” / “_arrogant_” and thus their sentiment score is -1. These words are marked as “SILVER” in the provenance column in _LatinAffectus_ which means they have been automatically annotated by some NLP model, without a human making the decision of setting the polarity score. The automated annotation can be an explanation why this result seems a bit odd. 

The case of “_fastus_” is a good example of a possible problem with using sentiment analysis. Latin is quite notorious of words having multiple, sometimes even contradictory meanings. Usually, this problem has been solved with looking at the context or the “bigger picture” where such words occur. However, in this case that is not possible since the analysis is done by using computational methods and each word is processed individually and thus the context is lost. This means that the work done by humans is still very much needed. One could try to mark the possibly ambiguous words in the wall inscriptions somehow and then go through their results manually to check how well they were detected and make corrections if needed. Another possibility could be to manually determine and list the polarities and polarity scores of these kinds of words based on their context at the very start. This of course would increase the work amount, especially when dealing with bigger datasets. 

The two wall inscriptions with the highest scores CIL IV 4182 (+2) and CIL IV 4197 (+3.5) also need some inspection. The wall inscription with the highest score is CIL IV 4197 and it has been mentioned previously concerning the repetition of words and how to handle them. Since the wall inscription is about greeting a person named Serenus its score probably should not be so high, and the score is the result of the repetition of a +1 adjective. 

CIL IV 4182’s sentiment score consists of _Augustus_ (+1, noun), _Caesar_ (+0.5, noun) and _consul_ (+0.5, noun). This wall inscription is about the market days in the cities of Cumae and Pompeii, and one could argue that should it actually get an exceptionally high positive score. There are no clearly visible emotions as it seems to be a simple ad about the upcoming market days. The sentiment score is made up of the nouns about the high-ranking members of the Roman society: _consul_ and the two nouns related to the title of the emperor (_Augustus_ and _Caesar_). _Augustus_ and _Caesar_ have been marked as “SILVER” in their provenance in _LatinAffectus_ which means that they have been automatically annotated and this might partly explain their high scores.

So, it would seem that CIL IV 4182 and CIL IV 4197 should have not gotten such high sentiment scores when the wall inscriptions are viewed as a whole and the context is taken into account. This once again highlights the importance of the human factor in the process. 

I do believe that sentiment analysis and the division to negative and positive sentiments can be useful in the study of wall inscriptions, even though there are some clear problems. The ambiguity of some words, as mentioned above, is one of the biggest ones which need to be tackled in the future. However, even the crude not 100 % correct division to negative and positive sentiments could reveal some interesting hidden patterns or at least point us to a new direction in which to focus our research on. 
